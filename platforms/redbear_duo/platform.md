...
2. Download the two signed firmware binaries [BLE_BlinkFast.zip](demo/BLE_BlinkFast.zip) and [BLE_BlinkSlow.zip](demo/BLE_BlinkSlow.zip), as well as the binary payload in [BLE_Blink_Fast.bin](demo/BLE_Blink_Fast.bin) to your phone.  You may want to email them to yourself in order to get these files to your phone.
...
7. If desired, repeat the steps to switch between the [BLE_BlinkFast.zip](demo/BLE_BlinkFast.zip) and [BLE_BlinkSlow.zip](demo/BLE_BlinkSlow.zip) payloads.
...
Start by cloning the [nrfutil](https://github.com/Trellis-Logic/pc-nrfutil) library and using the instructions to install on your system.
...
```
3. Use the instructions above with the nRF Toolbox application to download your firmware image to the Redbear Duo.  You should now see your application running on the Duo.
